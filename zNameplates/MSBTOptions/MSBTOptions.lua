...
local SCROLL_AREA_MOVER_PET_INCOMING_FRAME_NAME = "MSBTScrollAreaMoverPetIncomingFrame";
local SCROLL_AREA_MOVER_PET_OUTGOING_FRAME_NAME = "MSBTScrollAreaMoverPetOutgoingFrame";
...
local MIN_TEXT_SPEED = 25;
local MAX_TEXT_SPEED = 300;
local MIN_FADE_DELAY = 0;
local MAX_FADE_DELAY = 10;
...
function MikSBTOpt.InitDropdowns()
 ...
 availableScrollAreas["PetIncoming"] = {Name="Pet Incoming"};
 availableScrollAreas["PetOutgoing"] = {Name="Pet Outgoing"};
...
function MikSBTOpt.InitCheckboxes()
 ...
 key = "Tab2FramePetAreaCheckbox";	MikSBTOpt.SetupCheckbox(OPTIONS_FRAME_NAME .. key, "Enable Pet Incoming Area", "Enables combat events affecting your pet in their separate area.", "DisplaySettings.PetIncoming.Show");
 key = "Tab3FramePetAreaCheckbox";	MikSBTOpt.SetupCheckbox(OPTIONS_FRAME_NAME .. key, "Enable Pet Outgoing Area", "Enables combat events caused by your pet in their separate area.", "DisplaySettings.PetOutgoing.Show");
...
function MikSBTOpt.InitSliders()
 -- Initialize time-based scrolling controls.
 local key = "Tab1FrameAnimationStepSlider"; 
 MikSBTOpt.SetupSlider(OPTIONS_FRAME_NAME .. key, MikSBTOpt.SLIDERS[key].Label, MikSBTOpt.SLIDERS[key].Tooltip, MIN_TEXT_SPEED, MAX_TEXT_SPEED, 5, "TextSpeed");

 key = "Tab1FrameFadeDelaySlider";
 MikSBTOpt.SetupSlider(OPTIONS_FRAME_NAME .. key, MikSBTOpt.SLIDERS[key].Label, MikSBTOpt.SLIDERS[key].Tooltip, MIN_FADE_DELAY, MAX_FADE_DELAY, 0.5, "FadeDelay");
...
function MikSBTOpt.InitButtons()
 ...
 key = "Tab2FramePetFontSettingsButton";		MikSBTOpt.SetupButton(OPTIONS_FRAME_NAME .. key, "Pet Incoming Fonts", "Edits font settings for the Pet Incoming scroll area.");
 key = "Tab3FramePetFontSettingsButton";		MikSBTOpt.SetupButton(OPTIONS_FRAME_NAME .. key, "Pet Outgoing Fonts", "Edits font settings for the Pet Outgoing scroll area.");
...
function MikSBTOpt.ButtonOnClick()
 ...
 elseif (this:GetName() == OPTIONS_FRAME_NAME .. "Tab2FramePetFontSettingsButton") then
  MikSBTOpt.HideMessageEditbox(OPTIONS_FRAME_NAME .. "Tab2FrameMessageEditbox");
  local fontSettingsFrame = getglobal(FONT_SETTINGS_FRAME_NAME);
  fontSettingsFrame:SetPoint("TOPLEFT", this, "BOTTOMLEFT");
  fontSettingsFrame:Raise();
  getglobal(FONT_SETTINGS_FRAME_NAME .. "Title"):SetText("Pet Incoming");
  MikSBTOpt.ShowScrollAreaFontSettings("PetIncoming");

 elseif (this:GetName() == OPTIONS_FRAME_NAME .. "Tab3FramePetFontSettingsButton") then
  MikSBTOpt.HideMessageEditbox(OPTIONS_FRAME_NAME .. "Tab3FrameMessageEditbox");
  local fontSettingsFrame = getglobal(FONT_SETTINGS_FRAME_NAME);
  fontSettingsFrame:SetPoint("TOPLEFT", this, "BOTTOMLEFT");
  fontSettingsFrame:Raise();
  getglobal(FONT_SETTINGS_FRAME_NAME .. "Title"):SetText("Pet Outgoing");
  MikSBTOpt.ShowScrollAreaFontSettings("PetOutgoing");
 ...
 -- Pet incoming/outgoing event font settings buttons.
 elseif (this:GetParent().EventArea == "PetIncoming" or this:GetParent().EventArea == "PetOutgoing") then
  local eventArea = this:GetParent().EventArea;
  if (eventArea == "PetIncoming") then
   MikSBTOpt.HideMessageEditbox(OPTIONS_FRAME_NAME .. "Tab2FrameMessageEditbox");
  else
   MikSBTOpt.HideMessageEditbox(OPTIONS_FRAME_NAME .. "Tab3FrameMessageEditbox");
  end
  ...
  if (this:GetParent().EventType ~= nil) then
   getglobal(FONT_SETTINGS_FRAME_NAME .. "Title"):SetText(this:GetParent().FontFrameTitle);
   MikSBTOpt.ShowEventFontSettings(eventArea, this:GetParent().EventType);
  end
 ...
 for i = 1, EVENT_ITEMS_PER_PAGE do
  ...
  if (objEventSettings) then
   ...
   -- There is data for the event settings slot.
    ...
    if (string.find(objEventSettings.EventType, "INCOMING_PET_")) then
     objEventSettings.EventArea = "PetIncoming";
    else
 ...
 for i = 1, EVENT_ITEMS_PER_PAGE do
  ...
  if (objEventSettings) then
   ...
   -- There is data for the event settings slot.
    ...
    -- Set the font frame title to the checkbox's label and the event area.
    objEventSettings.FontFrameTitle = evtData.Label;
    if (string.find(objEventSettings.EventType, "OUTGOING_PET_")) then
     objEventSettings.EventArea = "PetOutgoing";
    else
 ...
 for i = 1, EVENT_ITEMS_PER_PAGE do
  ...
  if (objEventSettings) then
   ...
   -- There is data for the event settings slot.
    ...
    -- Set the font frame title to the checkbox's label and the event area.
    objEventSettings.FontFrameTitle = evtData.Label;
    objEventSettings.EventArea = "Notification";
...
function MikSBTOpt.ShowMasterFontSettings()
 getglobal(FONT_SETTINGS_FRAME_NAME .. "FontOutlineDropdown"):Show();
...
function MikSBTOpt.UpdateFontPreviews()
 ...
 if (fontIndex == nil or MikSBT.AVAILABLE_FONTS[fontIndex] == nil) then fontIndex = 1; end
 ...
 if (outlineIndex == nil or MikSBT.AVAILABLE_OUTLINES[outlineIndex] == nil) then outlineIndex = 1; end
 ...
 if (getglobal(FONT_SETTINGS_FRAME_NAME .. "CritPreview"):IsShown()) then
  ...
  if (fontCritIndex == 0 or fontCritIndex == nil) then
   fontCritIndex = fontSettingsFrame.InheritedFontCritIndex;
  end
  if (fontCritIndex == nil or MikSBT.AVAILABLE_FONTS[fontCritIndex] == nil) then fontCritIndex = fontIndex; end
  ...
  if (outlineCritIndex == 0 or outlineCritIndex == nil) then
   outlineCritIndex = fontSettingsFrame.InheritedOutlineCritIndex;
  end
  if (outlineCritIndex == nil or MikSBT.AVAILABLE_OUTLINES[outlineCritIndex] == nil) then outlineCritIndex = outlineIndex; end
...
function MikSBTOpt.ShowScrollAreaMoverFrames(scrollArea)
 ...
 -- Save the original pet scroll area settings.
 controlFrame.MSBTOriginalSettings.PetIncomingAnimationStyle = MikSBT.GetOptionFromVarPath("DisplaySettings.PetIncoming.AnimationStyle");
 controlFrame.MSBTOriginalSettings.PetIncomingScrollDirection = MikSBT.GetOptionFromVarPath("DisplaySettings.PetIncoming.ScrollDirection");
 controlFrame.MSBTOriginalSettings.PetIncomingTextAlignIndex = MikSBT.GetOptionFromVarPath("DisplaySettings.PetIncoming.FontSettings.TextAlignIndex");
 controlFrame.MSBTOriginalSettings.PetIncomingScrollHeight = MikSBT.GetOptionFromVarPath("DisplaySettings.PetIncoming.ScrollHeight");
 controlFrame.MSBTOriginalSettings.PetIncomingXOffset = MikSBT.GetOptionFromVarPath("DisplaySettings.PetIncoming.FrameOffsets.x");
 controlFrame.MSBTOriginalSettings.PetIncomingYOffset = MikSBT.GetOptionFromVarPath("DisplaySettings.PetIncoming.FrameOffsets.y");
 controlFrame.MSBTOriginalSettings.PetOutgoingAnimationStyle = MikSBT.GetOptionFromVarPath("DisplaySettings.PetOutgoing.AnimationStyle");
 controlFrame.MSBTOriginalSettings.PetOutgoingScrollDirection = MikSBT.GetOptionFromVarPath("DisplaySettings.PetOutgoing.ScrollDirection");
 controlFrame.MSBTOriginalSettings.PetOutgoingTextAlignIndex = MikSBT.GetOptionFromVarPath("DisplaySettings.PetOutgoing.FontSettings.TextAlignIndex");
 controlFrame.MSBTOriginalSettings.PetOutgoingScrollHeight = MikSBT.GetOptionFromVarPath("DisplaySettings.PetOutgoing.ScrollHeight");
 controlFrame.MSBTOriginalSettings.PetOutgoingXOffset = MikSBT.GetOptionFromVarPath("DisplaySettings.PetOutgoing.FrameOffsets.x");
 controlFrame.MSBTOriginalSettings.PetOutgoingYOffset = MikSBT.GetOptionFromVarPath("DisplaySettings.PetOutgoing.FrameOffsets.y");
 ...
 local petIncomingFrame = getglobal(SCROLL_AREA_MOVER_PET_INCOMING_FRAME_NAME);
 local petOutgoingFrame = getglobal(SCROLL_AREA_MOVER_PET_OUTGOING_FRAME_NAME);
 ...
 petIncomingFrame.tooltipText = MikSBTOpt.FRAMES[SCROLL_AREA_MOVER_PET_INCOMING_FRAME_NAME].Tooltip;
 petOutgoingFrame.tooltipText = MikSBTOpt.FRAMES[SCROLL_AREA_MOVER_PET_OUTGOING_FRAME_NAME].Tooltip;
 ...
 petIncomingFrame:Show();
 petOutgoingFrame:Show();
...
function MikSBTOpt.HideScrollAreaMoverFrames()
 ...
 getglobal(SCROLL_AREA_MOVER_PET_INCOMING_FRAME_NAME):Hide();
 getglobal(SCROLL_AREA_MOVER_PET_OUTGOING_FRAME_NAME):Hide();
end
...
function MikSBTOpt.RepositionScrollAreaMoverFrames()
 ...
 petIncomingFrame:SetHeight(controlFrame.MSBTCurrentSettings.PetIncomingScrollHeight);
 petIncomingFrame:ClearAllPoints();
 petIncomingFrame:SetPoint("BOTTOM", UIParent, "CENTER", controlFrame.MSBTCurrentSettings.PetIncomingXOffset, controlFrame.MSBTCurrentSettings.PetIncomingYOffset);
 petOutgoingFrame:SetHeight(controlFrame.MSBTCurrentSettings.PetOutgoingScrollHeight);
 petOutgoingFrame:ClearAllPoints();
 petOutgoingFrame:SetPoint("BOTTOM", UIParent, "CENTER", controlFrame.MSBTCurrentSettings.PetOutgoingXOffset, controlFrame.MSBTCurrentSettings.PetOutgoingYOffset);
...
function MikSBTOpt.ColorizeScrollAreaMoverFrames(scrollArea)
 ...
 petIncomingFrame:SetBackdropColor(0.8, 0.8, 0.8, 1.0);
 petOutgoingFrame:SetBackdropColor(0.8, 0.8, 0.8, 1.0);
 ...
 elseif (scrollArea == "PetIncoming") then
  petIncomingFrame:SetBackdropColor(0.5, 0.05, 0.05, 1.0);
 elseif (scrollArea == "PetOutgoing") then
  petOutgoingFrame:SetBackdropColor(0.5, 0.05, 0.05, 1.0);
 end
end
...
function MikSBTOpt.PopulateScrollAreaMoverCoordinates()
 ...
 getglobal(SCROLL_AREA_MOVER_PET_INCOMING_FRAME_NAME .. "Title"):SetText("Pet Incoming (" .. controlFrame.MSBTCurrentSettings.PetIncomingXOffset .. ", " .. controlFrame.MSBTCurrentSettings.PetIncomingYOffset .. ")");
 getglobal(SCROLL_AREA_MOVER_PET_OUTGOING_FRAME_NAME .. "Title"):SetText("Pet Outgoing (" .. controlFrame.MSBTCurrentSettings.PetOutgoingXOffset .. ", " .. controlFrame.MSBTCurrentSettings.PetOutgoingYOffset .. ")");
...
function MikSBTOpt.SaveScrollAreaMoverCoordinates(frameName)
 ...
 elseif (frameName == SCROLL_AREA_MOVER_PET_INCOMING_FRAME_NAME) then
  controlFrame.MSBTCurrentSettings.PetIncomingXOffset = xOffset;
  controlFrame.MSBTCurrentSettings.PetIncomingYOffset = yOffset;
 elseif (frameName == SCROLL_AREA_MOVER_PET_OUTGOING_FRAME_NAME) then
  controlFrame.MSBTCurrentSettings.PetOutgoingXOffset = xOffset;
  controlFrame.MSBTCurrentSettings.PetOutgoingYOffset = yOffset;
 end
...
function MikSBTOpt.SaveScrollAreaSettings(settingsType)
 ...
 local petAreas = {"PetIncoming", "PetOutgoing"};
 for _, petArea in petAreas do
  MikSBT.SetOptionFromVarPath("DisplaySettings." .. petArea .. ".AnimationStyle", controlFrame[settingsType][petArea .. "AnimationStyle"]);
  MikSBT.SetOptionFromVarPath("DisplaySettings." .. petArea .. ".ScrollDirection", controlFrame[settingsType][petArea .. "ScrollDirection"]);
  MikSBT.SetOptionFromVarPath("DisplaySettings." .. petArea .. ".FontSettings.TextAlignIndex", controlFrame[settingsType][petArea .. "TextAlignIndex"]);
  MikSBT.SetOptionFromVarPath("DisplaySettings." .. petArea .. ".ScrollHeight", controlFrame[settingsType][petArea .. "ScrollHeight"]);
  MikSBT.SetOptionFromVarPath("DisplaySettings." .. petArea .. ".FrameOffsets.x", controlFrame[settingsType][petArea .. "XOffset"]);
  MikSBT.SetOptionFromVarPath("DisplaySettings." .. petArea .. ".FrameOffsets.y", controlFrame[settingsType][petArea .. "YOffset"]);
 end
...
function MikSBTOpt.ScrollAreaMoverPreviewOnClick()
 ...
 MikSBT.DisplayMessage("Pet Incoming Preview", MikSBT.DISPLAYTYPE_PET_INCOMING, false, 255, 165, 0);
 MikSBT.DisplayMessage("Pet Outgoing Preview", MikSBT.DISPLAYTYPE_PET_OUTGOING, false, 0, 255, 128);
end
...
function MikSBTOpt.PopulateProfileOptions()
 ...
 -- Populate the time-based animation controls.
 MikSBTOpt.PopulateSlider(OPTIONS_FRAME_NAME .. "Tab1FrameAnimationStepSlider");
 MikSBTOpt.PopulateSlider(OPTIONS_FRAME_NAME .. "Tab1FrameFadeDelaySlider");
 ...
 MikSBTOpt.PopulateCheckbox(OPTIONS_FRAME_NAME .. "Tab2FramePetAreaCheckbox");
 MikSBTOpt.PopulateCheckbox(OPTIONS_FRAME_NAME .. "Tab3FramePetAreaCheckbox");
