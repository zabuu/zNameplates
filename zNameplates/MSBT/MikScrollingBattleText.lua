...
-- Display types for incoming, outgoing, notification, and pet events.
MikSBT.DISPLAYTYPE_INCOMING			= 1;
MikSBT.DISPLAYTYPE_OUTGOING			= 2;
MikSBT.DISPLAYTYPE_NOTIFICATION		= 3;
MikSBT.DISPLAYTYPE_PET_INCOMING		= 4;
MikSBT.DISPLAYTYPE_PET_OUTGOING		= 5;
...
local animationMergeData = {
 ...
 ["Outgoing"]	= {["TimeSinceMerge"] = 0, UnmergedEvents={}, MergedEvents={}, IsOutgoing=true},
 ["PetIncoming"]	= {["TimeSinceMerge"] = 0, UnmergedEvents={}, MergedEvents={}},
 ["PetOutgoing"]	= {["TimeSinceMerge"] = 0, UnmergedEvents={}, MergedEvents={}, IsOutgoing=true},
};

-- Optional consumer for integrations that render MSBT's outgoing events in a
-- different frame system (for example pfUI nameplates).  Kept private so MSBT
-- remains independent of the consuming addon.
local outgoingNameplateHandler = nil;
local outgoingNameplateEnabled = nil;
local outgoingNameplateTargetResolver = nil;
 ...
 for _, mergeData in animationMergeData do
  ...
  if ((mergeData.TimeSinceMerge >= MERGE_DELAY_TIME) and not stillMerging) then
   ...
   for _, animationEvent in mergeData.MergedEvents do
    if (not MikSBT.RouteOutgoingNameplateEvent(animationEvent)) then
...
function MikSBT.OnUpdateAnimationFrame(frame)
 ...
 elseif (frameName == scrollAreas.PetIncoming.AssociatedFrame) then
  scrollArea = scrollAreas.PetIncoming;
 elseif (frameName == scrollAreas.PetOutgoing.AssociatedFrame) then
  scrollArea = scrollAreas.PetOutgoing;
...
function MikSBT.SetupAnimationScrollAreas()
 ...
 scrollAreas.PetIncoming = {};
 scrollAreas.PetOutgoing = {};
 ...
 scrollAreas.PetIncoming.AssociatedFrame = "MSBTFramePetIncoming";
 scrollAreas.PetOutgoing.AssociatedFrame = "MSBTFramePetOutgoing";
 scrollAreas.Notification.AssociatedFrame = "MSBTFrameNotification";

 -- The original addon only defines three animation frames in XML.  Build the pet
 -- frames here so this addition stays compatible with the classic 1.12 UI API.
 local petFrameNames = {scrollAreas.PetIncoming.AssociatedFrame, scrollAreas.PetOutgoing.AssociatedFrame};
 for _, petFrameName in petFrameNames do
  if (not getglobal(petFrameName)) then
   local petFrame = CreateFrame("Frame", petFrameName, UIParent);
   petFrame:SetWidth(24);
   petFrame:SetHeight(24);
   petFrame:SetFrameStrata("HIGH");
   petFrame:Hide();
   for x = 1, NUM_FONT_STRINGS do
    local fontString = petFrame:CreateFontString(petFrameName .. "Text" .. x, "ARTWORK", "MasterFont");
    fontString:SetPoint("BOTTOMLEFT");
   end
   petFrame:SetScript("OnUpdate", function() MikSBT.OnUpdateAnimationFrame(this); end);
  end
 end
 ...
 scrollAreas.PetIncoming.DisplaySettings = MikSBT.CurrentProfile.DisplaySettings.PetIncoming;
 scrollAreas.PetOutgoing.DisplaySettings = MikSBT.CurrentProfile.DisplaySettings.PetOutgoing;
...
function MikSBT.RepositionAnimationFrames()
 ...
 for _, scrollArea in scrollAreas do
  local frame = getglobal(scrollArea.AssociatedFrame);
  frame:ClearAllPoints();
  frame:SetPoint("BOTTOM", "UIParent", "CENTER", scrollArea.DisplaySettings.FrameOffsets.x, scrollArea.DisplaySettings.FrameOffsets.y);
 end
end
...
function MikSBT.MergeEvents(mergeData, numEvents)
  ...
  for x = 1, numEvents do
   -- Enhanced combat GUID events may be delivered just after the legacy chat
   -- event. Resolve again at merge time, after MSBT's normal 0.3s wait.
   if (mergeData.IsOutgoing and MikSBT.IsOutgoingNameplateHandlerActive()) then
    MikSBT.ResolveOutgoingNameplateTarget(mergeData.UnmergedEvents[x]);
   end
   ...
   for _, animationEvent in mergeData.MergedEvents do
    ...
    local sameNameplateTarget = (not mergeData.IsOutgoing) or
      (not MikSBT.IsOutgoingNameplateHandlerActive()) or
      (not MikSBT.IsOutgoingNameplateEvent(mergeData.UnmergedEvents[x])) or
      (mergeData.UnmergedEvents[x].TargetGUID ~= nil and animationEvent.TargetGUID ~= nil and
       mergeData.UnmergedEvents[x].TargetGUID == animationEvent.TargetGUID);
    if ((mergeData.UnmergedEvents[x].EventType == animationEvent.EventType) and
        (mergeData.UnmergedEvents[x].EffectName == animationEvent.EffectName) and sameNameplateTarget) then
     ...
     -- There is an effect name.
      ...
      if (animationEvent.Name ~= nil and not (mergeData.IsOutgoing and
          MikSBT.IsOutgoingNameplateHandlerActive() and MikSBT.IsOutgoingNameplateEvent(animationEvent))) then
...
function MikSBT.UpdateProfiles()
 ...
 for _, profile in MikSBT_Save.Profiles do
  ...
  if (profile.CreationVersion < 4.7) then
   -- New independent pet areas and time-based animation controls.
   profile.DisplaySettings.PetIncoming = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.DisplaySettings.PetIncoming);
   profile.DisplaySettings.PetOutgoing = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.DisplaySettings.PetOutgoing);
   profile.TextSpeed = (profile.AnimationStep or 1.5) / ANIMATION_SPEED;
   profile.FadeDelay = MikSBT.DEFAULT_CONFIG.FadeDelay;
   profile.CreationVersion = 4.7;
  end

  -- Repair partially migrated profiles without overwriting user choices.
  if (not profile.DisplaySettings.PetIncoming) then
   profile.DisplaySettings.PetIncoming = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.DisplaySettings.PetIncoming);
  end
  if (not profile.DisplaySettings.PetOutgoing) then
   profile.DisplaySettings.PetOutgoing = MikSBT.CopyTable(MikSBT.DEFAULT_CONFIG.DisplaySettings.PetOutgoing);
  end
  if (not profile.TextSpeed or profile.TextSpeed <= 0) then profile.TextSpeed = MikSBT.DEFAULT_CONFIG.TextSpeed; end
  if (profile.FadeDelay == nil) then profile.FadeDelay = MikSBT.DEFAULT_CONFIG.FadeDelay; end
...
function MikSBT.DispatchAnimationEvent(animationEvent)
 local nameplateOutgoing = MikSBT.IsOutgoingNameplateHandlerActive() and
  MikSBT.IsOutgoingNameplateEvent(animationEvent);
 if (nameplateOutgoing) then MikSBT.ResolveOutgoingNameplateTarget(animationEvent); end

 -- Check if the event is to be shown.
 if (animationEvent.EventSettings.Show and (animationEvent.ScrollArea.DisplaySettings.Show or nameplateOutgoing)) then
  ...
  elseif (animationEvent.ScrollArea == scrollAreas.PetIncoming) then
   table.insert(animationMergeData.PetIncoming.UnmergedEvents, animationEvent);
  elseif (animationEvent.ScrollArea == scrollAreas.PetOutgoing) then
   table.insert(animationMergeData.PetOutgoing.UnmergedEvents, animationEvent);
...
function MikSBT.AddAnimation(animationEvent)
  ...
  if MikSBT.CurrentProfile.ResistSound then
    if animationEvent.EffectName then
      if (string.find(animationEvent.EffectName, "Cone of Cold") or string.find(animationEvent.EffectName, "Frost Nova")) then
        PlaySoundFile("Interface\\AddOns\\zNameplates\\MSBT\\sounds\\Resist.mp3");
      end
    end
  end
 end
 ...
 animDisplayInfo.StartTime = animDisplayInfo.LastUpdated;
...
function MikSBT.DoAnimation(animDisplayInfo, scrollArea)
 ...
 local stickyDisplayTime = MikSBT.CurrentProfile.FadeDelay or STICKY_DISPLAY_TIME;
 ...
 if (animDisplayInfo.IsSticky or (animDisplayInfo.IsCrit and MikSBT.CurrentProfile.UseStickyCrits)) then
  ...
  if (elapsedTime <= STICKY_FADE_IN_TIME) then
   ...
   return;
  elseif (elapsedTime <= (stickyDisplayTime + STICKY_FADE_IN_TIME)) then
   animDisplayInfo.FontObject:SetTextHeight(animDisplayInfo.FontSize);
   if animDisplayInfo.Texture then
	animDisplayInfo.Texture:SetWidth(animDisplayInfo.FontSize);
    animDisplayInfo.Texture:SetHeight(animDisplayInfo.FontSize);
   ...
   return;
  elseif (elapsedTime <= (stickyDisplayTime + STICKY_FADE_IN_TIME + STICKY_FADE_OUT_TIME)) then
   local alpha = 1 - ((elapsedTime - stickyDisplayTime - STICKY_FADE_IN_TIME) / STICKY_FADE_OUT_TIME);
 ...
 if (elapsedTime > ANIMATION_SPEED) then
  -- Move by pixels per second so changing speed never changes travel distance and
  -- remains stable at different frame rates.
  local textSpeed = MikSBT.CurrentProfile.TextSpeed or MikSBT.DEFAULT_CONFIG.TextSpeed;
  animDisplayInfo.AnimationStep = textSpeed * elapsedTime;
...
function MikSBT.ScrollUp(animDisplayInfo, scrollArea)
 ...
 animDisplayInfo.PositionY = animDisplayInfo.PositionY + animDisplayInfo.AnimationStep;
 MikSBT.UpdateScrollingAlpha(animDisplayInfo, scrollArea);
...
-- **********************************************************************************
-- Sets alpha using elapsed time.  FadeDelay controls how long text remains fully
-- opaque; it then fades over the remaining travel time.
-- **********************************************************************************
function MikSBT.UpdateScrollingAlpha(animDisplayInfo, scrollArea)
 local elapsed = GetTime() - animDisplayInfo.StartTime;
 local fadeDelay = MikSBT.CurrentProfile.FadeDelay or 0;
 local textSpeed = MikSBT.CurrentProfile.TextSpeed or MikSBT.DEFAULT_CONFIG.TextSpeed;
 local travelTime = scrollArea.DisplaySettings.ScrollHeight / textSpeed;
 ...
 if (elapsed <= fadeDelay) then
  ...
  if (fadeTime <= 0) then
   ...
   animDisplayInfo.Alpha = 1 - ((elapsed - fadeDelay) / fadeTime);
   if (animDisplayInfo.Alpha < 0) then animDisplayInfo.Alpha = 0; end
  end
 end
end
...
-- **********************************************************************************
-- Registers a synchronous consumer for outgoing animation events.  The enabled
-- callback is checked at dispatch and merge time, so toggling the integration does
-- not require changing MSBT's own Outgoing.Show setting.
-- **********************************************************************************
function MikSBT.RegisterOutgoingNameplateHandler(handler, enabledCallback, targetResolver)
 if (type(handler) == "function") then
  outgoingNameplateHandler = handler;
  outgoingNameplateEnabled = enabledCallback;
  outgoingNameplateTargetResolver = targetResolver;
 else
  outgoingNameplateHandler = nil;
  outgoingNameplateEnabled = nil;
  outgoingNameplateTargetResolver = nil;
 end
end


function MikSBT.IsOutgoingNameplateHandlerActive()
 if (not outgoingNameplateHandler) then return false; end
 if (type(outgoingNameplateEnabled) ~= "function") then return true; end
 local ok, enabled = pcall(outgoingNameplateEnabled);
 return ok and enabled and true or false;
end


-- Damage and attack-result events belong over their affected nameplate.  Healing
-- and generic messages remain in MSBT's normal Outgoing area.
function MikSBT.IsOutgoingNameplateEvent(animationEvent)
 if (not animationEvent) then return false; end
 if (animationEvent.ScrollArea ~= scrollAreas.Outgoing and animationEvent.ScrollArea ~= scrollAreas.PetOutgoing) then
  return false;
 end
 return animationEvent.EventType ~= "MSBT_EVENTTYPE_OUTGOING_HEAL" and
  animationEvent.EventType ~= "MSBT_EVENTTYPE_OUTGOING_HOT" and animationEvent.EventType ~= nil;
end


function MikSBT.ResolveOutgoingNameplateTarget(animationEvent)
 if (animationEvent.TargetGUID or type(outgoingNameplateTargetResolver) ~= "function") then return; end
 local ok, targetGuid = pcall(outgoingNameplateTargetResolver, animationEvent);
 if (ok and targetGuid) then animationEvent.TargetGUID = targetGuid; end
end


-- Returns true when ownership of the event was transferred to the integration.
-- The callback is synchronous because animation events come from a recycler.
function MikSBT.RouteOutgoingNameplateEvent(animationEvent)
 if (not MikSBT.IsOutgoingNameplateHandlerActive()) then return false; end
 if (not MikSBT.IsOutgoingNameplateEvent(animationEvent)) then return false; end

 local ok = pcall(outgoingNameplateHandler, animationEvent);
...
-- Formats and checks an animation event against MSBT suppressions.  Integrations
-- use this before rendering so MSBT remains the source of text/filter behavior.
function MikSBT.PrepareExternalAnimationEvent(animationEvent)
 MikSBT.FormatEventText(animationEvent);
 if (not animationEvent.Text) then return false; end
 for _, suppressionSettings in MikSBT.CurrentProfile.Suppressions do
  if (suppressionSettings.Enabled and string.find(animationEvent.Text, suppressionSettings.SearchPattern)) then
   return false;
  elseif (suppressionSettings.Enabled and animationEvent.EffectName and string.find(animationEvent.EffectName, suppressionSettings.SearchPattern)) then
...
function MikSBT.DisplayMessage(message, displayType, isSticky, colorR, colorG, colorB, fontSize, fontIndex, outlineIndex, Texture)
 ...
 if (displayType ~= MikSBT.DISPLAYTYPE_INCOMING and displayType ~= MikSBT.DISPLAYTYPE_OUTGOING and
     displayType ~= MikSBT.DISPLAYTYPE_NOTIFICATION and displayType ~= MikSBT.DISPLAYTYPE_PET_INCOMING and
     displayType ~= MikSBT.DISPLAYTYPE_PET_OUTGOING) then
 ...
 elseif (displayType == MikSBT.DISPLAYTYPE_OUTGOING) then
  scrollArea = scrollAreas.Outgoing;
 elseif (displayType == MikSBT.DISPLAYTYPE_PET_INCOMING) then
  scrollArea = scrollAreas.PetIncoming;
 elseif (displayType == MikSBT.DISPLAYTYPE_PET_OUTGOING) then
  scrollArea = scrollAreas.PetOutgoing;
