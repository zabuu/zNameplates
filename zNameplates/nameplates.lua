function zNameplates.LoadNameplates()
  if zNameplates.nameplates then return end
  if pfNameplates then
    DEFAULT_CHAT_FRAME:AddMessage("|cffff5555zNameplates:|r Disable pfUI's nameplates module, then reload the UI.")
    return
  end

  local Runtime = zNameplates
  local C = zNameplates.config
  local strsplit = zNameplates.StrSplit
  local round = zNameplates.Round
  local Abbreviate = zNameplates.Abbreviate
  local GetStringColor = zNameplates.GetStringColor
  local GetBorderSize = zNameplates.GetBorderSize
  local CreateBackdrop = zNameplates.CreateBackdrop
  local GetUnitInfo = zNameplates.GetUnitInfo
  local PFUI_CLASS_COLORS = zNameplates.classColors
  local NAMEPLATE_OBJECTORDER = zNameplates.NAMEPLATE_OBJECTORDER
  local COOLDOWN_FRAME_TYPE = zNameplates.cooldownFrameType
  local CooldownFrame_OnUpdateModel = zNameplates.CooldownFrame_OnUpdateModel
  local UnitCreatureTypeID = UnitCreatureTypeID or function() return nil end
  ...
  local function MSBTAlignment()
    local align = zNameplates_Config.align or "CENTER"
  ...
  local function ConfigureMSBTFrame(plate)
    ...
    local height = tonumber(zNameplates_Config.height) or 80
    local x = tonumber(zNameplates_Config.x) or 0
    local y = tonumber(zNameplates_Config.y) or 8
  ...
  local function UpdateMSBTAnimations(plate, now)
    ...
    local height = frame.configHeight or tonumber(zNameplates_Config.height) or 80
    local speed = tonumber(MikSBT.CurrentProfile.TextSpeed) or 75
    local fadeDelay = tonumber(zNameplates_Config.fade) or .5
  ...
  local function UpdateDebuffConfig(nameplate, i)
    ...
    local font = C.nameplates.use_unitfonts == "1" and Runtime.font_unit or Runtime.font_default
...
nameplates:RegisterEvent("ADDON_LOADED")
  ...
  -- Register once immediately and once after this bundled add-on initializes.
  ...
  nameplates:SetScript("OnEvent", function()
    ...
    elseif event == "ADDON_LOADED" and arg1 == "zNameplates" then
      RegisterMSBTIntegration()
  ...
  nameplates.OnCreate = function(frame)
    ...
    nameplate.glow:SetTexture(Runtime.media["img:dot"])
    nameplate.glow:Hide()
    ...
    if C.unitframes.blizzard_raidicons ~= "1" then
      nameplate.raidicon:SetTexture(Runtime.media["img:raidicons"])
    end
  ...
  nameplates.OnConfigChange = function(frame)
    ...
    local hptexture = Runtime.media[C.nameplates.healthtexture]
    ...
    local cbtexture = Runtime.media[C.appearance.castbar.texture]
  ...
  nameplates.OnUpdate = function(frame, state)
    ...
    local throttle
    if target then
      throttle = Runtime.throttle:Get("nameplates_target")
    elseif visiblePlateCount > 20 then
      throttle = Runtime.throttle:Get("nameplates_mass")
    else
      throttle = Runtime.throttle:Get("nameplates")
    end

    -- Non-target plates with active castbar use the castbar throttle
    if isCastingNonTarget then
      local cbThrottle = Runtime.throttle:Get("nameplates_castbar")
    ...
    if cfg.showcastbar and not cfg.targetcastbar and not isTargetPlate then
      ...
      if visiblePlateCount > 20 then
        local massThrottle = Runtime.throttle:Get("nameplates_mass")
